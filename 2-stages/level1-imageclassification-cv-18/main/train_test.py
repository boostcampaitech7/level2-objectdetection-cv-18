...
def train_test():
    ...
    for fold in range(k_folds):
        ...
        np.save(os.path.join(test_csv_dir, f"fold{fold+1}_eva_giant_mlp_gelu.npy"), predictions)
        
        csv_name_fold = f"fold{fold+1}_eva_giant_mlp_gelu.csv"
    ...
    np.save(os.path.join(test_csv_dir, f"all_fold_eva_giant_mlp_gelu.npy"), k_fold_predictions)
    ...
    np.save(os.path.join(test_csv_dir, f"softvoting_all_fold_eva_giant_mlp_gelu.npy"), average_probs)
