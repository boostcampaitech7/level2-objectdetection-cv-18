...
def detect(source, weights, img_size=640, conf_thres=0.25, iou_thres=0.45):
    ...
    # CSV 파일 열기
    with open('output_final.csv', mode='w', newline='') as csv_file:
        ...
        for data in dataset:
            ...
            if img is not None:  # img가 None이 아닐 때만 처리
                ...
                if pred.ndim == 3:  # pred가 3D인 경우
                    if pred.shape[2] == 15:  # Check the number of attributes
                        pred = pred[0]  # Select the first (and only) batch item
                    else:
                        print(f"Unexpected prediction shape: {pred.shape}")
                        continue  # Or handle it as needed
                elif pred.ndim == 2:  # pred가 2D인 경우
                    pred = pred.unsqueeze(0)  # 차원 추가
